...
const ProjectCard = ({ id, title, image, short, isDark }) => (
  <div className={`shadow-lg rounded-2xl overflow-hidden transform hover:scale-[1.02] transition duration-300 max-w-lg w-full
...
const Projects = ({ isDark, setIsDark }) => {
  ...
  const projectId = location.pathname.startsWith('/projects/') ? location.pathname.split('/projects/')[1] : null
  const selectedProject = projects.find((p) => p.id === projectId)

  return (
    <div className={`${isDark ? 'bg-black text-white' : 'bg-white text-black'} flex flex-col min-h-screen`}>
      <NavBar isDark={isDark} setIsDark={setIsDark}/>
      <div className='flex-grow px-4 lg:px-6 mt-[80px] lg:mt-[80px] w-full overflow-auto'>
        <div className='max-w-6xl mx-auto text-center'>
          <h1 className='text-4xl font-bold mb-4'>Projects</h1>
          <p className={`mb-10 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Some of my favorite and most impactful work.</p>
